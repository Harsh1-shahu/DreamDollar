...
const Login = () => {
            ...
            if (response.ok && (data.ResponseStatus === "success" || data.ResponseCode === 101)) {
                ...
                if (typeof parsed === "string") {
                    try {
                        ...
                        setLoading(false);
                        setToast({ message: "Bad response format from server.", type: "error" });
                        return;
                    }
                }
                ...
                if (row && typeof row === "object") {
                    ...
                    // 👉 Add custom value if logging in by username
                    if (byUsernameOnly) {
                        userPayload.DirectLogin = true;   // or whatever key/value you want
                        userPayload.Note = "Logged in with username only";
                    }
                    ...
                    setToast({ message: "No user data returned.", type: "error" });
                }
            } else {
                setLoading(false);
                setErrors((prev) => ({
    ...
    return (
        <div className="flex items-center justify-center min-h-screen bg-gray-900 p-2 relative">
            ...
            {toast.message && (
                <div
                    className={`fixed top-5 left-1/2 transform -translate-x-1/2 px-6 py-3 rounded-lg shadow-lg text-white text-sm font-medium transition-all duration-500 ${toast.type === "success" ? "bg-green-600" : "bg-red-600"
                        }`}
                >
                    {toast.message}
                </div>
