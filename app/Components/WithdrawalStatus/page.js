      ...
      <div className="mt-14 mb-20 p-4">
        ...
        <div className="flex justify-start mb-4">
          <input
            ...
            className={`w-full px-3 py-2 rounded-full border focus:outline-none focus:ring focus:ring-cyan-300 ${theme === "dark"
        ...
        {filteredData.length > 0 && (
          <div
            className={`flex justify-between items-center mt-6 p-2 rounded-lg shadow-md font-semibold ${theme === "dark"
              ? "bg-gray-800 text-gray-100"
              : "bg-white text-gray-900"
              }`}
          >
            <p className="w-[30vw] sm:w-full">Total Amount: {totalAmount.toFixed(2)}</p>
            <p className="w-[40vw] sm:w-full text-cyan-500 text-right">Total Final Amount: {totalFinalAmount.toFixed(2)}</p>
          </div>
        )}
            ...
            <div className="space-y-4 mt-4">
              {currentItems.length > 0 ? (
                currentItems.map((item, idx) => (
                  <div
                    key={idx}
                    className={`flex justify-between items-center p-2 rounded-lg shadow-sm ${theme === "dark" ? "bg-gray-800" : "bg-white"
                      }`}
                  >
