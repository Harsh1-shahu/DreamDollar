...
const EarningReport = () => {
  ...
  // 🔹 Get latest balance (by latest date)
  const latestRecord = filteredData.reduce((latest, row) => {
    const currentDate = new Date(row?.TTTIME);
    if (!latest || currentDate > new Date(latest?.TTTIME)) {
      return row;
    }
    return latest;
  }, null);

  const latestAmount = latestRecord ? parseFloat(latestRecord?.BALANCE ) || 0 : 0;
      ...
      >
        ...
        <div className="flex items-center justify-between mb-4">
          <input
            ...
            className={`w-[52vw] sm:w-[25vw] border rounded-full px-4 py-1 focus:outline-none focus:ring focus:ring-cyan-300 ${theme === "dark"
                  ...
                  >
                    ...
                    <div className="w-full flex justify-between">
                      <div className="flex items-center space-x-3 mb-2 md:mb-0">
                        ...
                        <div>
                          <p className="text-sm w-30 font-medium">{item.REMARK || "-"}</p>
