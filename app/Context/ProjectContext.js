...
export const ProjectProvider = ({ children }) => {
  ...
  const requestWithdrawal = async (direct, rank, auto, gold) => {
    try {
      setLoading(true);
      setError("");

      // Always ensure 4 numbers are sent
      const values = [direct, rank, auto, gold].map(v => Number(v) || 0);
      const txtrequestamt = values.join(","); // "0,0,9,10"

      const formData = new FormData();
      formData.append("memb_code", "101"); // TODO: replace with logged-in user
      formData.append("type", "WithdrawalMember");
      formData.append("txtrequestamt", txtrequestamt);
      ...
      if (result.ResponseStatus === "success") {
        ...
        return { success: true, data: parsedData, message: result.ResponseMessage };
      } else {
